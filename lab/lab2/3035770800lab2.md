...
add x10, x11, X12
```
There are 4 machine instructions in total.
...
3. `li   a1, 65535`
    ...
    add x10, x11, x12
    ```
- When is constant is less or equal to 11 bits, then the instruction `li` will be assembled into two instructions: `lui` and `addi`, because the constant can be loaded using a single `addi` instruction.
